class Global
  ...
  class << self
    ...
    def daemon_statuses
      Rails.cache.fetch('coinroots:daemons:statuses', expires_in: 3.minute) do
  ...
  def key(key, interval=5)
    ...
    "coinroots:#{@currency}:#{key}:#{time_key}"
  end

  def asks
    Rails.cache.read("coinroots:#{currency}:depth:asks") || []
  end

  def bids
    Rails.cache.read("coinroots:#{currency}:depth:bids") || []
  end
  ...
  def ticker
    ticker           = Rails.cache.read("coinroots:#{currency}:ticker") || default_ticker
    open = Rails.cache.read("coinroots:#{currency}:ticker:open") || ticker[:last]
  ...
  def trades
    Rails.cache.read("coinroots:#{currency}:trades") || []
  end
